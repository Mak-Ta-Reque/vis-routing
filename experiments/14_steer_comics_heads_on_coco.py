...
so the only thing that changes is which heads are used to steer.
"""
import sys
...
comics_heads = [(r["layer"], r["head"]) for r in comics_ranking[:TOP_K_HEADS]]
comics_by_layer = group_heads_by_layer(comics_heads)
print(f"Comics-discovered top-{TOP_K_HEADS} heads: {comics_heads[:10]}...")
print(f"Across {len(comics_by_layer)} layers.")
...
demo_samples = find_multi_object_images(coco_index, N_DEMO_SAMPLES, seed=SEED + 1)   # SAME seed as before -> same 10 images
print(f"\nSame {len(demo_samples)} demo images as the COCO-discovered-heads run:")
for s in demo_samples:
    print(f"  image_id={s['image_id']}  original={s['original']['category']!r}  alternate={s['alternate']['category']!r}")
...
def caption_with_steering(image, prompt, target_bbox_xyxy, heads_by_layer, n_query_heads):
    ...
    cell_mask, target_positions = bbox_to_token_positions(target_bbox_xyxy, grid_shape, image.size, img_start)
...
results = []
for sample in tqdm(demo_samples, desc="Steering demo [comics heads]"):
    ...
    steered_text = caption_with_steering(image, CAPTION_PROMPT, alt_bbox_xyxy, comics_by_layer, n_heads)
...
print(f"\nComics-heads steering redirected the caption to the alternate object in {n_match}/{len(results)} samples ({100*n_match/len(results):.0f}%).")

dump_json(REPO_ROOT / "logs" / "steer_demo_comics_heads" / "summary.json", results)
print("\nDONE")
