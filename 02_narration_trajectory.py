...
heads. Vis heads produce a staircase that climbs panel by panel in sync with
the generated text; control heads show no panel structure.

Outputs per comic (under figures/narration_trajectory/<comic>/):
  - vis_head_trajectory.pdf
  - non_vis_heads_trajectory.pdf
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--top-k-vis-head", type=int, default=100)
...
def main() -> None:
    ...
    for comic_dir in comic_dirs:
        ...
        save_figure_pdf(
            ...
            comic_fig_dir / "vis_head_trajectory.pdf",
        )
        save_figure_pdf(
            build_trajectory_figure(
                ...
                transitions=transitions,
                attention_mode=args.attention_mode,
                title=f"Random Non-Vis Heads (n={len(control_heads)})",
                shared_vmax=shared_vmax,
            ),
            comic_fig_dir / "non_vis_heads_trajectory.pdf",
        )
