#!/usr/bin/env python3
"""Discover vis heads from six-panel comic strips.
...
head gets a 6x6 queried-panel x attended-panel matrix; the visual-routing score is its
...
Outputs (under logs/<output-name>/ and figures/<output-name>/):
  - vis_head_ranking.json   ranked (layer, head, score) list
  - vis_head_scores.npy          (n_layers, n_heads) score matrix
  - mean_panel_attention.npy (n_panels, n_layers, n_heads, n_panels)
  - vis_head_score_histogram.pdf, vis_head_score_map.pdf
  - vis_head.pdf, non_vis_heads.pdf (example 6x6 matrices)

Usage:
    # panel-folder comics (comicN/p1.png..p6.png)
    python 01_discover_vis_heads.py --comics-root /path/to/comics --n-samples 500
    ...
    python 01_discover_vis_heads.py --use-raw --comics-root /path/to/raw_panel_images --n-samples 500
"""
...
from vis_head.regions import assign_panels_to_tokens


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Discover vis heads from six-panel comic strips.")
    ...
    parser.add_argument("--output-name", type=str, default="vis_head_discovery")
...
def select_random_non_vis_heads(
...
def main() -> None:
    ...
    top_vis_head = ranked_heads[: max(1, args.top_k_preview)]
    random_non_vis_heads = select_random_non_vis_heads(
    ...
    save_figure_pdf(
        plot_score_histogram(vis_head_scores, top10_threshold, top100_threshold),
        outputs.figures_dir / "vis_head_score_histogram.pdf",
    )
    save_figure_pdf(plot_score_map(vis_head_scores), outputs.figures_dir / "vis_head_score_map.pdf")
    save_figure_pdf(
        plot_head_heatmaps_grid(mean_panel_attention, top_vis_head, "Vis Heads"),
        outputs.figures_dir / "vis_head.pdf",
    )
    save_figure_pdf(
        plot_head_heatmaps_grid(mean_panel_attention, random_non_vis_heads, "Non-Vis Heads"),
        outputs.figures_dir / "non_vis_heads.pdf",
    )
