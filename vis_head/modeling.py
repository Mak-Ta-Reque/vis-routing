"""Qwen-VL model loading, input preparation, and generation helpers.

Supports the Qwen2-VL, Qwen2.5-VL, and Qwen3-VL families (the model class is
picked automatically from the checkpoint's `model_type`). The model is always
loaded with eager attention so per-head attention weights are accessible, and
so the attention-mask steering hooks see the mask kwarg.
"""
...
# model_type (from AutoConfig) -> transformers class name to load.
_MODEL_TYPE_TO_CLASS_NAME = {
...
def _resolve_model_class(model_id: str) -> Any:
    """Pick the right `...ForConditionalGeneration` class for `model_id`."""
    import transformers
    from transformers import AutoConfig

    config = AutoConfig.from_pretrained(model_id)
    model_type = getattr(config, "model_type", None)
    class_name = _MODEL_TYPE_TO_CLASS_NAME.get(model_type)
    if class_name is None:
        raise ValueError(
            f"Unsupported model_type={model_type!r} for {model_id!r}. "
            f"Expected one of {sorted(_MODEL_TYPE_TO_CLASS_NAME)}."
        )
    try:
        return getattr(transformers, class_name)
    except AttributeError as exc:
        raise ImportError(
            f"transformers has no {class_name}; upgrade transformers to load {model_id!r}."
        ) from exc
...
def load_model_and_processor(
    ...
    """Load a Qwen-VL model + processor with eager attention.

    Works with any Qwen2-VL / Qwen2.5-VL / Qwen3-VL checkpoint id.
    """
    from transformers import AutoProcessor

    model_class = _resolve_model_class(model_id)
...
def model_dims(model: Any) -> tuple[int, int, int]:
    ...
    Qwen3-VL nests the language-model config under `cfg.text_config`; Qwen2-VL
    and Qwen2.5-VL put those fields directly on the top-level config. Try the
    nested form first and fall back to the flat one.
    """
    cfg = model.config
    ...
    spatial_merge = int(getattr(vision_cfg, "spatial_merge_size", 2)) if vision_cfg is not None else 2
    return n_layers, n_heads, spatial_merge


def language_model_layers(model: Any) -> torch.nn.ModuleList:
    """Return the LM transformer blocks where vis heads live.

    Transformers releases have nested the Qwen-VL language model differently
    across Qwen2-VL / Qwen2.5-VL / Qwen3-VL, so we try the known paths and
    accept the first that yields a ModuleList.
    """
    candidate_paths = [
