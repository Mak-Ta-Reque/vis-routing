...
def assign_grid_cells_to_tokens(
    image_grid_thw,
    rows: int,
    cols: int,
    spatial_merge: int,
) -> tuple[np.ndarray, tuple[int, int, int]]:
    """2D analogue of `assign_panels_to_tokens` for an `ImageNetGrid`
    (vis_head/imagenet_grid.py): splits the merged token grid into `rows` x
    `cols` equal-sized bands (cells are equal pixel size on the image side,
    unlike comic panels' variable widths) and assigns each token a
    row-major cell id `row * cols + col`.

    Returns (cell_ids, grid_shape) where cell_ids has one entry per LM image
    token, row-major over the merged (t, h, w) grid.
    """
    t, merged_h, merged_w = get_merged_grid_shape(image_grid_thw, spatial_merge)
    row_boundaries = _monotonic_boundaries(merged_h, [1] * rows)
    col_boundaries = _monotonic_boundaries(merged_w, [1] * cols)

    row_of_row_idx = np.empty((merged_h,), dtype=int)
    for r in range(rows):
        row_of_row_idx[row_boundaries[r]:row_boundaries[r + 1]] = r
    col_of_col_idx = np.empty((merged_w,), dtype=int)
    for c in range(cols):
        col_of_col_idx[col_boundaries[c]:col_boundaries[c + 1]] = c

    cell_grid = row_of_row_idx[:, None] * cols + col_of_col_idx[None, :]   # (merged_h, merged_w)
    cell_ids = np.tile(cell_grid.reshape(-1), t)
    return cell_ids, (t, merged_h, merged_w)
