...
def word_jaccard_similarity(a: str, b: str) -> float:
    """Word-level Jaccard similarity in [0, 1] between two normalized texts.

    1.0 = identical (after normalization) or both empty; 0.0 = disjoint
    vocabularies. The continuous counterpart of `steered_matches_baseline`'s
    threshold test — use this when a binary "did it change" flag saturates
    (e.g. a strong intervention that flips almost every sample) and a graded
    effect size is needed instead.
    """
    ...
    return (len(a_set & b_set) / union) if union else 1.0


def steered_matches_baseline(steered: str, baseline: str, jaccard_threshold: float = 0.85) -> bool:
    """Detect when steering had essentially zero effect (the non-vis-head control
    signature): normalized exact match or word-level Jaccard >= threshold."""
    if baseline is None:
        return False
    return word_jaccard_similarity(steered, baseline) >= jaccard_threshold


# ---------------------------------------------------------------------------
# Semantic (NLI) similarity — a model-independent alternative to
# word_jaccard_similarity for causal-effect measurement. Word overlap can
# under- or over-count effect size for reasons that have nothing to do with
# meaning (paraphrase, word order, synonym choice), and differs systematically
# in scale between models with different output "style" (e.g. base vs.
# instruct-tuned checkpoints), which makes cross-model effect-size comparisons
# unreliable. A fixed NLI model — trained specifically to judge whether two
# sentences say the same thing — gives an actual meaning-based signal; plain
# mean-pooled sentence embeddings (e.g. raw RoBERTa) were tried first and
# rejected: their anisotropy makes nearly every pair of sentences cosine-
# similar >0.99 regardless of content, which carries no signal at all.
# ---------------------------------------------------------------------------

_DEFAULT_SEMANTIC_MODEL_ID = "microsoft/deberta-xlarge-mnli"
_semantic_encoder_cache: dict[tuple[str, str], tuple[Any, Any]] = {}


def _get_semantic_encoder(model_id: str, device: str) -> tuple[Any, Any]:
    key = (model_id, device)
    if key not in _semantic_encoder_cache:
        from transformers import AutoModelForSequenceClassification, AutoTokenizer

        # use_safetensors=True: some cached MNLI checkpoints only ship a legacy
        # pytorch_model.bin on their default revision, which recent
        # transformers refuses to torch.load for security reasons (CVE-2025-32434)
        # unless torch>=2.6. Forcing safetensors resolves to a revision that has one.
        tokenizer = AutoTokenizer.from_pretrained(model_id)
        model = AutoModelForSequenceClassification.from_pretrained(model_id, use_safetensors=True).to(device)
        model.eval()
        _semantic_encoder_cache[key] = (tokenizer, model)
    return _semantic_encoder_cache[key]


def _nli_probs(tokenizer, model, premise: str, hypothesis: str, device: str):
    import torch

    inputs = tokenizer(premise, hypothesis, return_tensors="pt", truncation=True, max_length=128).to(device)
    with torch.no_grad():
...
def semantic_similarity(
    ...
    b: str,
    device: str = "cpu",
    model_id: str = _DEFAULT_SEMANTIC_MODEL_ID,
) -> float:
    """Symmetric NLI-based semantic similarity in [0, 1] between texts `a` and
    `b`, from a fixed encoder (default: DeBERTa-xlarge-MNLI) independent of
    whichever VLM produced the texts, so effect sizes are comparable across
    different model checkpoints.

    similarity = P(entailment) + 0.5 * P(neutral), averaged over both
    (a as premise, b as hypothesis) and the reverse direction: 1.0 when the
    two texts mutually entail each other (say the same thing), ~0.5 for
    unrelated/neutral content, and near 0 when they contradict each other.
    Empty-vs-empty is defined as 1.0 (identical), empty-vs-non-empty as 0.0.
    """
    ...
    tokenizer, model = _get_semantic_encoder(model_id, device)
    id2label = {int(k): v.lower() for k, v in model.config.id2label.items()}
    entail_idx = next(i for i, label in id2label.items() if "entail" in label)
    contra_idx = next(i for i, label in id2label.items() if "contradict" in label)
    neutral_idx = next(i for i in id2label if i not in (entail_idx, contra_idx))

    def directional_score(premise: str, hypothesis: str) -> float:
        probs = _nli_probs(tokenizer, model, premise, hypothesis, device)
        return float(probs[entail_idx] + 0.5 * probs[neutral_idx])

    forward = directional_score(a_norm, b_norm)
    backward = directional_score(b_norm, a_norm)
