#!/usr/bin/env python3
"""Causal VQA steering: redirect vis heads to a target panel and judge the answer.

For every (strip, target_panel) pair:
  1. Generate a baseline VQA answer with no intervention.
  2. Steer the top-K vis heads' attention onto the target panel
...
def main() -> None:
    ...
    cutoff_score = float(np.percentile(vis_head_scores, args.nonvishead_percentile))
    random_heads = sample_non_vis_heads(
        n_layers=n_layers,
        n_heads=n_query_heads,
        exclude=set(vis_head_top),
        n_select=args.top_k_random,
        seed=args.seed,
        scores=vis_head_scores,
        max_score=cutoff_score,
    )
    print(
        f"Non-Vis-Head sampling: percentile={args.nonvishead_percentile}, "
