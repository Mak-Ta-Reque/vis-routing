#!/usr/bin/env python3
"""Causal static narration steering: an ambiguous "this panel" prompt, resolved by the vis heads.
...
summary of the whole strip. With the vis heads steered to a target panel,
...
def main() -> None:
    ...
    ranking_path = (
        Path(args.vis_head_ranking)
        if args.vis_head_ranking
        else outputs.logs_dir.parent / "vis_head_discovery" / "vis_head_ranking.json"
    )
    ...
    vis_head_top = load_head_ranking(ranking_path, top_k=args.top_k_vis_head)
    cutoff = float(np.percentile(vis_head_scores, args.nonvishead_percentile))
    random_heads = sample_non_vis_heads(
        n_layers=n_layers, n_heads=n_query_heads, exclude=set(vis_head_top),
        n_select=args.top_k_random, seed=args.seed,
        scores=vis_head_scores, max_score=cutoff,
    )
    conditions = {
        f"vis_head_top{args.top_k_vis_head}": group_heads_by_layer(vis_head_top),
        f"non_vis_head_{args.top_k_random}": group_heads_by_layer(random_heads),
    }
    ...
    print(f"Non-Vis-Head cutoff (raw score percentile {args.nonvishead_percentile}) = {cutoff:.4f}", flush=True)
